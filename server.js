...
let getPrimaryCollectionName = async function(collectionName) {
	let collection = await itemCollectionModel.findOne({ $or: [ { 'name': collectionName }, { 'aliases': { { value: collectionName }} ]})
...
let getPrimaryItemName = async function(itemName) {
	let item = await itemModel.findOne({ $or: [ { name: itemName }, { 'aliases': { { value: itemName }} ]})
